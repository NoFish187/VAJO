window.onload = () => {
    ...
    let gameRunning = false; // Flag, um den Spielstatus zu verfolgen
    let countdownInterval;

    function startGame() {
        startPopup.style.display = "none"; // Popup ausblenden
    ...
    function updateScoreHistory() {
        const scoreHistoryLbl = document.getElementById("score-history");
        scoreHistoryLbl.innerHTML = "Score History: " + scoreHistory.join(", ");
    }
        ...
        if (tetromino) {
            ...
            if (tetromino.collides(i => ({ x: tetromino.x[i], y: tetromino.y[i] + 1 }))) {
                ...
                if (completedRows) {
                    ...
                    lines += completedRows;
                    scoreHistory.push(score); // Add the current score to history
                    updateScoreHistory(); // Update the displayed score history
                } else {
